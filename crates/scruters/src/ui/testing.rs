use crate::state::{
    testing::{
        ...
        tests::Test,
        ActiveComponent, OutputSource, TestingState,
    },
    State,
};
            ...
            for test in group.tests() {
                match test {
                    Test::IntegrationTarget {
                        ...
                        let target_group = Group::AutoGenerated(AutoGeneratedGroup::empty_from_metadata(AutoGeneratedGroupMetadata {
                            package_name: package_name.to_owned(),
                            target_kind: TargetKind::Test(target_name.to_owned()),
                        }));

                        let group = testing_state
                            .groups
                        ...
                        if let Some(group) = group {
                            let mut group_status = Some(true);
                            'group_tests: for test in group.tests() {
                                match testing_state
                                    .tests_output
                                    .get(test)
                                    .and_then(|test_result| {
                                        test_result.passed
                                    }) {
                                    None if matches!(
                                        group_status,
